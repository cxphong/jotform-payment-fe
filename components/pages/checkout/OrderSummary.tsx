...
const OrderSummary = ({ checkout, checkoutState }: Props) => {
    const allowOrderToBePlaced = checkoutState.paymentInfoCompleted
    ...
    const totalPrice = checkout ? checkout.cart.subtotal : 0
        ...
        <div className="col-span-2 border border-slate-100 rounded-lg p-4 flex-row space-y-3 h-min mb-6 md:mb-0 bg-white shadow-sm">
            ...
            <div className="flex justify-between pt-2">
                <h2 className="font-medium text-sm">
                    Items ({checkout && checkout.cart ? checkout.cart.totalItems : 0})
                </h2>
                <h1 className="font-medium text-sm">
                    ${checkout && checkout.cart ? checkout.cart.subtotal : 0}
                </h1>
            </div>
